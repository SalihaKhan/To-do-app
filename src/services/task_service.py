...
from persistence.storage import FileStorageManager, load_tasks_from_file, save_tasks_to_file
...
class TaskService:
    ...
    def __init__(self, data_file_path: str = DEFAULT_DATA_FILE):
        ...
        Args:
            data_file_path: Path to the data file for persistence
        """
        self._data_file = data_file_path
        self._storage_manager = FileStorageManager(data_file_path)

        # Load existing tasks from file if they exist
        self._tasks: List[Task] = self._storage_manager.load_tasks()
    ...
    def save_tasks(self):
        ...
        """
        self._storage_manager.save_tasks(self._tasks)

    def load_tasks(self):
        """
        Load all tasks from the data file.
        """
        self._tasks = self._storage_manager.load_tasks()

        # Update next ID based on loaded tasks
        if self._tasks:
            self._next_id = max(task.id for task in self._tasks) + 1
        else:
            self._next_id = 1
